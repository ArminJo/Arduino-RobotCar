 ...
 */
void EncoderMotorControl::syncronizeMotor(EncoderMotorControl * aOtherMotorControl, uint16_t aCheckInterval) {
...
// walk through list
        while (tEncoderMotorControlPointer != NULL) {
            if (tEncoderMotorControlPointer->MinSpeed == 0) {
                tEncoderMotorControlPointer->DCMotor->setSpeed(i);
                tEncoderMotorControlPointer->ActualSpeed = i;
            }
            tEncoderMotorControlPointer = tEncoderMotorControlPointer->NextMotorControl;
        }
        delay(100);
...
//            }
            ...
            if (tEncoderMotorControlPointer->MinSpeed == 0 && tEncoderMotorControlPointer->DistanceCount > 6) {
                tEncoderMotorControlPointer->MinSpeed = i;
                tEncoderMotorControlPointer->StopSpeed = i;
            }
            if (tEncoderMotorControlPointer->MinSpeed == 0) {
 ...
 *****************************************************************************************/
/*
 * aIntPinNumber can be one of INT0 or INT1 for 328
 */
void EncoderMotorControl::enableInterruptOnRisingSlope(uint8_t aIntPinNumber) {
    if (aIntPinNumber > 1) {
        return;
    }
